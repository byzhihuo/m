...
(function($, window, document, undefined) {
	...
	function triggerVirtualEvent(eventType, event, flags) {
		...
		if((flags && flags[eventType]) ||
			...
			jQuery(event.target).trigger(ve);
		}
		return ve;
	}
	...
	function getSpecialEventObject(eventType) {
		...
		return {
			setup: function(data, namespace) {
				...
				jQuery(this).bind(realType, dummyMouseHandler);
		...
		setup : function() {
			...
			obj.on('vmousedown', function(e) {
				...
				function cleartaphandlers() {
					...
					jQuery(document).off('vmousecancel', cleartaphandlers);
				}
				...
				jQuery(document).on('touchcancel', cleartaphandlers);
...
var page = {
	converthtml : function() {
		var prevpage = jQuery('div.pg .prev').prop('href');
		var nextpage = jQuery('div.pg .nxt').prop('href');
		var lastpage = jQuery('div.pg label span').text().replace(/[^\d]/g, '') || 0;
		var curpage = jQuery('div.pg input').val() || 1;

		if(!lastpage) {
			prevpage = jQuery('div.pg .pgb a').prop('href');
		}
		...
		jQuery('div.pg').removeClass('pg').addClass('page').html('<a href="'+ prevpagehref +'">上一页</a>'+ selector +'<a href="'+ nextpagehref +'">下一页</a>');
		jQuery('#dumppage').on('change', function() {
			var href = (prevpage || nextpage);
			window.location.href = href.replace(/page=\d+/, 'page=' + jQuery(this).val());
		});
	},
};
	...
	init : function(obj) {
		...
		jQuery(window).on('resize', function() {
			if(fixed) {
				...
				obj.css('top', (jQuery(document).scrollTop() + jQuery(window).height() - 40) + 'px');
			}
		});
		obj.on('tap', function() {
